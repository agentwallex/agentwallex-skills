...
allowed-tools:
  ...
  - Bash(npx @agentwallex/cli@0.1.0 topup*)
...
3. **Get the deposit address.** Run `npx @agentwallex/cli@0.1.0 topup --chain <chain> --token <token>` (or `npx @agentwallex/cli@0.1.0 account deposit --chain <chain> --token <token>`).
...
5. **Optionally check balance after deposit.** The user can verify funds arrived by running `npx @agentwallex/cli@0.1.0 account balance`.
...
| Command                                                                     | Description                          |
|-----------------------------------------------------------------------------|--------------------------------------|
| `npx @agentwallex/cli@0.1.0 topup --chain <chain> --token <tkn>`           | Get a deposit address                |
| `npx @agentwallex/cli@0.1.0 topup --chain <chain> --json`                  | Get deposit address with JSON output |
...
```bash
npx @agentwallex/cli@0.1.0 topup --chain base --token USDC
```
